...
import sys
import os
sys.path.append('/home/developer/PROJECTS/Ecrash/DataAugmentationForObjectDetection-master/data_aug/')
...
class Resize(object):
    ...
    def __init__(self, inp_dim1,inp_dim2):
        self.inp_dim_w = inp_dim1
        self.inp_dim_h = inp_dim2
        
    def __call__(self, img, bboxes):
        w,h = img.shape[1], img.shape[0]
        img = letterbox_image(img, self.inp_dim_w, self.inp_dim_h)
        
    
        scale = min(self.inp_dim_h/h, self.inp_dim_w/w)
        bboxes[:,:4] *= (scale)
    
        new_w = scale*w
        new_h = scale*h
        inp_dim_w = self.inp_dim_w   
        inp_dim_h = self.inp_dim_h 
